...
PSID sspi::GetUserSidFromWellKnownRid(DWORD Rid)
{
   ...
   if((pSid = (PSID)LocalAlloc(LPTR,
          GetSidLengthRequired((UCHAR)(ucSubAuthorityCount + 1)))) != nullptr)
   {
      if (InitializeSid(
            pSid,
            GetSidIdentifierAuthority(umi2->usrmod2_domain_id),
            (BYTE)(ucSubAuthorityCount+1)
            ))
      {
         ...
         {
            *GetSidSubAuthority(pSid, dwSubAuthIndex) =
            *GetSidSubAuthority(umi2->usrmod2_domain_id, dwSubAuthIndex);
         }
      ...
      PVOID pIn, DWORD cbIn, PVOID pOut, PDWORD pcbOut, PBOOL pfDone) 
{
   ...
   if(!pAS->bInitialized) 
   {
      ss = m_AcquireCredentialsHandle(nullptr, _T("NTLM"),
            SECPKG_CRED_OUTBOUND, nullptr, pAuthIdentity, nullptr, nullptr,
            &pAS->hcred, &tsExpiry);
      /**/
      if (ss < 0)
         return FALSE;
   ...
   ss = m_InitializeSecurityContext(&pAS->hcred,
         pAS->bInitialized ? &pAS->hctxt : nullptr, nullptr, 0, 0,
         SECURITY_NATIVE_DREP, pAS->bInitialized ? &sbdIn : nullptr,
